class_name SVGControllerViewport
...
func _props_applied(changed_props = []):
	...
	if (view_box.size.x == 0 or view_box.size.y == 0) and not is_root_element:
	...
	if attr_height is SVGLengthPercentage:
		if attr_height.percentage == null or not is_root_element:
	...
	if attr_width is SVGLengthPercentage:
		if attr_width.percentage == null or not is_root_element:
